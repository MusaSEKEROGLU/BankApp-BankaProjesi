...
{
    public partial class Customer1HesapListesi : System.Web.UI.Page
    {
        ...
        {   //Customer1-HesaplarıGetir
            ...
            var value = JsonConvert.DeserializeObject<List<BBANKCustomer1Account>>(response);
        ...
        {   //checkBox tekli Seçim yapma
            ...
            for (int i = 0; i < GrdHesaplar.Rows.Count; i++)
            {
                ...
                if (((CheckBox)row.FindControl("chkHesaplar")).Checked)
                {
                    ...
                    var request1 = httpClient1.GetAsync("https://localhost:44340/api/BBANKCustomer1Account1Details/BBANKGetAllCustomer1Account1Details").Result;
                    var response1 = request1.Content.ReadAsStringAsync().Result;
                    var value1 = JsonConvert.DeserializeObject<List<BBANKCustomer1Account1Details>>(response1);
                    ...
                    connection.Open();
                    SqlCommand cmd = new SqlCommand("Select * FROM BBANKCustomer1Account1Activities", connection);
            ...
            for (int i = 1; i < GrdHesaplar.Rows.Count; i++)
            {
                ...
                {
                    var httpClient1 = new HttpClient();
                    var request1 = httpClient1.GetAsync("https://localhost:44340/api/BBANKCustomer1Account2Details/BBANKGetAllCustomer1Account2Details").Result;
                    var response1 = request1.Content.ReadAsStringAsync().Result;
                    var value1 = JsonConvert.DeserializeObject<List<BBANKCustomer1Account2Details>>(response1);
                    GrdHesapDetay.DataSource = value1;
                    GrdHesapDetay.DataBind();
                    //Hesap Hareketleri Customer1 Hesap2 
                    SqlConnection connection = new SqlConnection("Data Source=DESKTOP-THSSRI7\\SQLEXPRESS;Initial Catalog=Dbo_MusteriBankaHesabı;Integrated Security=True;");
                    connection.Open();
                    SqlCommand cmd = new SqlCommand("Select * FROM BBANKCustomer1Account2Activities", connection);
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    DataTable dt = new DataTable();
                    adapter.Fill(dt);
                    GrdHesapHareketleri.DataSource = dt;
