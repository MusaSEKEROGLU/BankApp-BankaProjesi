        ...
        {   //checkBox tekli Seçim yapma
            ...
            for (int i = 0; i < GrdHesaplar.Rows.Count; i++)
            {
                ...
                if (((CheckBox)row.FindControl("chkHesaplar")).Checked)
                {
                    ...
                    var request1 = httpClient1.GetAsync("https://localhost:44340/api/ABANKCustomer2Account1Details/ABANKGetAllCustomer2Account1Details").Result;
                    var response1 = request1.Content.ReadAsStringAsync().Result;
                    var value1 = JsonConvert.DeserializeObject<List<ABANKCustomer2Account1Details>>(response1);
                    ...
                    connection.Open();
                    SqlCommand cmd = new SqlCommand("Select * FROM ABANKCustomer2Account1Activities", connection);
            ...
            for (int i = 1; i < GrdHesaplar.Rows.Count; i++)
            {
                ...
                {
                    var httpClient1 = new HttpClient();
                    var request1 = httpClient1.GetAsync("https://localhost:44340/api/ABANKCustomer2Account2Details/ABANKGetAllCustomer2Account2Details").Result;
                    var response1 = request1.Content.ReadAsStringAsync().Result;
                    var value1 = JsonConvert.DeserializeObject<List<ABANKCustomer2Account2Details>>(response1);
                    GrdHesapDetay.DataSource = value1;
                    GrdHesapDetay.DataBind();
                    //Hesap Hareketleri Customer1 Hesap2 
                    SqlConnection connection = new SqlConnection("Data Source=DESKTOP-THSSRI7\\SQLEXPRESS;Initial Catalog=Dbo_MusteriBankaHesabı;Integrated Security=True;");
                    connection.Open();
                    SqlCommand cmd = new SqlCommand("Select * FROM ABANKCustomer2Account2Activities", connection);
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    DataTable dt = new DataTable();
                    adapter.Fill(dt);
                    GrdHesapHareketleri.DataSource = dt;
