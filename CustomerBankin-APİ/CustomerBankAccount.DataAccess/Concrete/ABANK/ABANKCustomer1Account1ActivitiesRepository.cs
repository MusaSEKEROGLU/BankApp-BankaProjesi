...
{
    public class ABANKCustomer1Account1ActivitiesRepository : ABANKICustomer1Account1ActivitiesRepository
    {
        ABANKCustomer1Account1Activities customer1Account1Activities = new ABANKCustomer1Account1Activities();
        public ABANKCustomer1Account1Activities Customer1Account1ActivitiesById(int id)
        {
            SqlDataAdapter adapter = new SqlDataAdapter("sp_ABANKCustomer1Account1ActivitiesById_Select", CBADbContext.Baglanti);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                customer1Account1Activities.GelenTransferTutarı = Convert.ToDecimal(dt.Rows[0]["GelenTransferTutarı"]);
        ...
        {
            SqlDataAdapter adapter = new SqlDataAdapter("sp_ABANKCustomer1Account1Activities_Select", CBADbContext.Baglanti);
            adapter.SelectCommand.CommandType = CommandType.StoredProcedure;
            DataTable dt = new DataTable();
            adapter.Fill(dt);
            List<ABANKCustomer1Account1Activities> listcustomer1Account1Activities = new List<ABANKCustomer1Account1Activities>();
            if (dt.Rows.Count > 0)
            {
